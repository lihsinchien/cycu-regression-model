...
The coefficient of bmi $\approx$ 0.7

$$  \log \frac{p_B}{1-p_B} - \log \frac{p_A}{1-p_A} = (36.6-27.5) \times \hat{\beta} =9.7\cdot \hat{\beta} \approx 0.642$$
$$ \frac{\frac{p_B}{1-p_B}}{\frac{p_A}{1-p_A}} \approx e^{0.642} \approx 1.9 $$

$$  se(9.7\hat{\beta}) =9.7\cdot se(\hat{\beta}) $$

The 95% CI for $9.7  \hat{\beta}$ would be


```{r}
se.beta<-sqrt(summary(r)$cov.unscaled[7,7])
ci.diff<-9.7*r$coefficient[7]+c(-1,1)*qnorm(0.975)*9.7*se.beta
...
假設其他變數值固定的情況下，bmi 從 1st quantile (27.5) 增加到 3rd quantile (36.6), 則 the log odds of testing positive for diabetes 將增加 0.642, 95% 信賴區間為 (0.164,1.204)

或可解釋為: 假設其他變數值固定的情況下，bmi 從 1st quantile (27.5) 增加到 3rd quantile (36.6), 則 the odds of testing positive for diabetes 將為原來的 1.9 倍, 95% 信賴區間為 (1.1793.334)。
